...
**Product intent:** [`PRODUCT-IDEAS.md`](PRODUCT-IDEAS.md). UI sketches are **not locked**; operator notes override. Do not document layout choices as final or confirmed.  
...
4. **Visual reference:** [`mc-manager-ui-mockup.html`](mc-manager-ui-mockup.html) is the look to copy (status cluster, power buttons, pinned stats, whitelist Add-IP popup + hover row actions, Server Management four info cards + backup rows, section cards, tabs). Mockup is dark. B2 shipped a light warm-gray remap; **operator later rejected that light theme** — live product tokens are **twilight granite + cobalt** in `wwwroot/css/app.css` (also match the WPF window background). Novice Status = **Running/Stopped** only (**green / red** text); no `?` help on Status / Play IP / Players. Technical VM/door status on Advanced. Players = `—` when Minecraft is off. Power buttons are filled (Start moss / Stop redstone / Restart cobalt) and grey with **no hover-react** when they would not work. Save-type buttons disabled until a change would actually push. Pinned stat cards must **not** grow with window width and must share one height that matches the status card + power row. Mini-bars follow stat-text width. Rollover bank = unused hours from earlier days this month (Core `UsageMath` leftover), explained only via the card `?` — do not repeat that caption on the card. Debug host probes live on **Advanced** (Debug builds), not a global overlay. Default and minimum window width hug the chrome row (~786 DIP client); extra width **centers** the fixed shell instead of stretching it. `MainWindow` remeasures the WebView2 client on load so min-width cannot clip the right pad. Vertical scroll is **tab-body only** (status/pins/tabs stay put). The tab-body scrollbar sits in the **right window gutter** (thin overlay-style thumb) so overflowing tabs do not shift cards left of the chrome row.
...
- [`PRODUCT-IDEAS.md`](PRODUCT-IDEAS.md): desktop-stack line currently “.NET + Avalonia” → .NET + Blazor Hybrid (WPF + WebView2). Changelog + “Old / superseded” row for Avalonia-as-UI-vehicle (keep the Wails→Avalonia row as history).
...
- Lab [`V1-Implementation-Plan.md`](Development-Steps.md) and [`VM-Software.md`](VM-Software.md) only as needed for “what is built” / suggested order (Phase B before Guide/E2E).
...
2026-08-16 — DEBUG probes moved off the global overlay onto **Advanced** (still Debug-only via `UiHostProbes.Enabled`).
...
| 2026-08-17 | Manage tab-body scrollbar lives in the right window gutter (thin overlay-style thumb); tab cards stay chrome-width with or without overflow. `MinWidth` remeasures WebView2 client so left/right pads stay even. |
| 2026-08-16 | Accent → cobalt (`#4d8ef5`); Usage tab hero metrics + grouped budget; “game computer” → “server” in UI help copy. |
| 2026-08-16 | Window default/`MinWidth` hugs the manage chrome (~786 DIP client); extra width centers a fixed shell (no stretch). |
