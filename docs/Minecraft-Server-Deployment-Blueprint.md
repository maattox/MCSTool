...
**This is a firm, operator-confirmed product decision, not a staging note that might loosen at v1 or later.** The Manager/Setup wizard **never** implements a Modrinth/CurseForge/FTB browse, search, "trending packs," or "type a pack name and we look it up" UI — in v1, and not planned for any later stage either, unless the operator explicitly revisits it. This governs the *selection* UX specifically, and has one direct, load-bearing consequence for how Part B's per-platform sections must be read and implemented:
...
4. **No CurseForge API key needed at all until CurseForge-format import specifically ships.** Because there is no browse feature, this product's CurseForge dependency is scoped exclusively to "resolve file URLs referenced inside an uploaded pack" (§23) — a strictly smaller, later-startable integration than a full catalog integration would have required.
...
| `white-list` | `true` | MVP is private-only; combined with OCI Security List allowlisting, not a substitute for it |
| `enforce-whitelist` | `true` | Rejects already-connected players promptly if removed from whitelist mid-session |
...
**Stage: v1 candidate for the *technical* integration, gated hard on legal/ToS acceptance before shipping** (this research surfaces real, current restrictions that must be designed around, not discovered after integration is "done").

**Reminder (§2.4): this is file-import support, not a CurseForge browse/search integration.** The user exports/downloads a modpack (ideally its published "server pack," see §24.1) from CurseForge's own website/app and uploads that archive into Setup. Every API call described below exists solely to resolve **download URLs for files that archive's own `manifest.json` already names** — this product still never calls a CurseForge search/listing endpoint, never presents CurseForge pack results in its UI, and never lets a user pick a pack by ID/name from inside this app. This scoping is also what keeps this integration clear of CurseForge's "no competing product" ToS clause (§23.2) — the product is resolving files for a pack the user already chose on CurseForge's own platform, not re-implementing pack discovery.
...
| **CurseForge pack import** (file upload/drag-and-drop of an already-exported pack; **not** an in-app CurseForge browser) | **v1 candidate, gated on API-key approval + legal/ToS design work** | Confirmed via research: technically important (huge, often-exclusive pack catalog) but now requires an approved API key (enforcement began 2026-07-16) purely to resolve file download URLs referenced by an uploaded pack's manifest, has explicit no-cache/no-proxy/no-competing-product ToS constraints that shape key custody, and needs a maintained client-only-mod heuristic (no per-file server marker exists in the format). **Ship after Modrinth, as its own explicitly-scoped work item, not bundled into "implement modpack support" as a single estimate.** |
...
This document is research + architecture, not an execution checklist — [`OCI-mc-server/docs/MVP-Implementation-Plan.md`](MVP-Implementation-Plan.md) remains the living execution checklist agents must follow step-by-step. This section only maps this document's sections onto that plan's existing/likely future steps so an implementing agent knows exactly where to start.

| MVP-Implementation-Plan step | What this document supplies |
|---|---|
