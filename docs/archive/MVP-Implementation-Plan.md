...
**Product intent:** **Operator will** is the source of truth. [`PRODUCT-IDEAS.md`](PRODUCT-IDEAS.md) is vision/roadmap (MVP section), **not infallible**. This file is an **archive** (Phases 0–7 DONE). Do not silently rewrite living docs to match PRODUCT-IDEAS; ask, or follow the current operator-requested document and note drift.  
**Suggested narrative order:** lab [`V1-Implementation-Plan.md`](Development-Steps.md).  
**Live infra docs:** lab repo (`Infrastructure-Information.md`, `VM-Software.md`).  
...
6. **Fix the product path, not only the test VM.** If troubleshooting the blank-tenancy / Setup deploy shows a bug caused by OpenTofu, IAM matching rules, cloud-init, SSH bootstrap, `onbox/mcmgr/`, `door_vm/`, or `vm_agent/` install: file it in lab `Issues.md` **and** change the automated-deploy code in the same effort so the next greenfield run does not repeat it. Patching only the live test instance is not done. Example: SETUP-ISSUE-2 (door DG tag match + compartment-only `manage public-ips`) had to land in product HCL, not just a Console tweak.
7. **`ubuntu` often cannot read/write the files you need.** Recurring pitfall (lab `Agent-Deploy-Pitfalls.md`): `/etc/mccontrol/oci.env` is mode 600 root; `/opt/mcmgr/`, `/etc/mcmgr/`, `/etc/mc-manager/`, systemd units, and many scripts are root-owned. Before operating on a path as `ubuntu`, check permissions; use `sudo` or fix ownership/mode. Do not burn a session rediscovering `Permission denied`.
8. **UI sketches in PRODUCT-IDEAS are not locked; operator UI notes override.** See [Phase 6](#phase-6--ui-polish), [Phase B](#phase-b--blazor-hybrid-ui), and `PRODUCT-IDEAS.md` → Manager UI. Do **not** build a mini-terminal / console status panel. Novice Status is Running/Stopped; technical VM/door status is on Advanced. For UI-design work, use or offer the `find-skills` skill unless the operator already asked; also look at panels such as Pterodactyl for feature reference. **NuGet is allowed** on the Manager UI project (`McManager.Hybrid`). Search for themes, icons, controls, fonts, or other UI needs. Do not add Avalonia / Semi / Material.Icons.Avalonia packages. Keep OCI SDK on Core; prefer OSS licenses; ask before paid/commercial packages; confirm large IA redesigns with the operator.
...
- Update lab `VM-Software.md` Avalonia row to “manage MVP usable”.
...
- Triage lab `Issues.md` for MVP blockers only (e.g. door SoftStop skipping world backup OS-ISSUE-6 if success criteria require backup on all stop paths).
...
- Confirm CHDIR with lab [`Operator-Troubleshooting.md`](Operator-Troubleshooting.md) (`systemctl cat`, `namei -l`). File leftover findings in [`Issues.md`](Issues.md).
...
- Update lab [`Operator-Troubleshooting.md`](Operator-Troubleshooting.md) + product [`Agent-Deploy-Pitfalls.md`](Agent-Deploy-Pitfalls.md) with the contract (not only the CHDIR symptom).
...
- **Also implement (or explicitly defer with operator OK) one-shots for other known failure modes** — design from lab `Issues.md` + Step 4.1 findings. Candidate set:
...
- Update lab [`Operator-Troubleshooting.md`](Operator-Troubleshooting.md) so each new button maps to the SSH/OCI commands it runs.
...
- **Operator (not agents):** after declaring MVP complete, run the clean-room acceptance test in `PRODUCT-IDEAS.md` (new account + installer + full Setup + $1 budget stress). Prefer a local VM / spare PC. This test may incur the documented ~$1–$2 residual — do not run it on the long-lived lab tenancy unless that spend is explicitly accepted.
...
| MVP / v1 intent | `PRODUCT-IDEAS.md` |
...
| Suggested order narrative | Lab `V1-Implementation-Plan.md` |
| What’s live on VMs today | Lab `VM-Software.md` |
| OCI layout | Lab `Infrastructure-Information.md` |
| Door behavior | Lab `Door-VM-Control-Plane.md` |
| Operator SSH/OCI troubleshooting commands | Lab [`Operator-Troubleshooting.md`](Operator-Troubleshooting.md) |
| Known bugs / quirks | Lab `Issues.md` |
| Deploy pitfalls (SSH/sudo; `ubuntu` permissions) | Lab `Agent-Deploy-Pitfalls.md` |
...
| OCI API usage (429, waiters, thrift) | `OCI-API-Usage.md` (lab twin: `OCI-mc-server-manager/docs/OCI-API-Usage.md`) |
...
- Paid / spend mode (**not v1** — later / far future; V1 Phase 8 SKIPPED)  
...
| 2026-08-15 | Inserted **Phase 4** (stabilize test stack: idle/`wait_forge` investigation, in-game whitelist off, Manager one-shot repairs). Former Phases 4–8 → **5–9**. NEXT = Step 4.1. Phases 1–3 unchanged. Operator runbook: lab `Operator-Troubleshooting.md`. |
