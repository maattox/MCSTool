...
**Dates:** 2026-08-19 (S0, S1, S2, S3, **S4**). **Paused after S2** for OS-ISSUE-9; **P1 DONE** 2026-08-19; **P2 DONE** 2026-08-19 (S2-11); **P3 DONE** 2026-08-19 (S2-16–18). **S3 DONE** 2026-08-19 (one **Minor** Fail: S3-04 leftover `/24`). **S4 DONE** 2026-08-19 (one **Major** Fail suggested: S4-12 name/icon/MOTD not applied). Operator may continue **S5**. Do not start 9.1.  
...
|----|--------|----------|-------|
| S5-01 | | | |
| S5-02 | | | first-kick may be Known DOOR-ISSUE-1 |
| S5-03 | | | |
| S5-04 | | | skip if S2-09 Pass |
| S5-05 | | | optional |
...
| ID | Result | Severity | Notes |
|----|--------|----------|-------|
| S7-02 | | | optional; restore shape |
| S7-03 | | | optional |
| S7-04 | | | skip unless operator authorized tofu |
...
6. **Wipe auto-start (question, not a Fail):** Operator expected Minecraft to start again after Wipe world. Catalog / Guide / V1 Step 1.3: stop and **leave stopped** until Start/Restart. Operator Restart generated a new world (Pass S3-07). Treat auto-start as a UX preference until triage.
...
| S3-04 | Yes (Minor) | *(not in Pass 1 P1–P3)* | Leftover Minecraft **prefix** CIDR after allowlist revert (`IsManagedRule` `/32`-only). SL restored. |
| S3-07 auto-start | Question | — | Operator wants Restart after wipe; current product leaves Minecraft stopped. Not a catalog Fail. |
| S4-11 modded half | Defer | — | Vanilla empty-state Pass. Live Modding inspect waits for a later modded redeploy. |
| S4-12 | Yes (Major suggested) | *(not in Pass 1 P1–P3)* | Name/description/icon never appeared in the Java server browser after save + start/restart (including save while already running). Confirm severity at triage. |

Pass 1 bug-fix plan: [`V1-Bug-Fix-Plan-Pass-1.md`](V1-Bug-Fix-Plan-Pass-1.md) (**P1–P3 DONE**). **S4 DONE** (one Major Fail suggested: S4-12; S3-04 Minor leftover still open). Operator may continue catalog **S5**. Do not start 9.1.
