...
4. If you change a test VM or TESTING cloud resource, make the **same** change in local SoT (`onbox/`, `infra/`, `door_vm/`, `vm_agent/`, `functions/shutdown_vm/`, Manager/Setup). File lab [`Issues.md`](Issues.md) for on-box/Setup/door bugs.  
...
8. **Operator will:** this plan is the operator-requested execution doc for Pass 1 remaining fixes. If it disagrees with `PRODUCT-IDEAS.md`, **follow this plan** and note the drift (do **not** rewrite this file to match PRODUCT-IDEAS). Stop and ask only if you cannot tell which document the operator meant.
...
Operator confirmed the suggested severities except **S3-07** (must auto-start after wipe; do not keep leave-stopped). **This plan is operator will** for these items. `PRODUCT-IDEAS.md` **Wipe world (v1)** still says the next Start creates a world — **do not rewrite P8 back to that**. Note the drift; update Guide / catalog expected in P8.
...
**Status:** DONE  
**Catalog IDs:** S2-05 (related), S2-08 notes, additional problem #3 → [OS-ISSUE-9](Issues.md)  
...
- Lab [`Issues.md`](Issues.md) — **OS-ISSUE-5**, **OS-ISSUE-9**, **SETUP-ISSUE-7**  
- [`infra/cloud-init/vm1.yaml.tftpl`](../infra/cloud-init/vm1.yaml.tftpl)  
- [`Agent-Deploy-Pitfalls.md`](Agent-Deploy-Pitfalls.md) (firewalld / netfilter)  
- Lab [`Operator-Troubleshooting.md`](Operator-Troubleshooting.md) — VM1 STOPPING / firewalld blocks  
...
- `PRODUCT-IDEAS.md` heading **Usage budget behavior (MVP+)** — one paragraph: *After daily exhaustion stop: only admin via Manager; door refuses player wake*  
...
**PRODUCT-IDEAS drift:** [`PRODUCT-IDEAS.md`](PRODUCT-IDEAS.md) **Wipe world (v1)** step 4 still says stop, then the *next* Start creates a world. Pass 1 catalog / Guide / V1 Step 1.3 said leave stopped. **Operator will (this plan):** after wipe, Minecraft **starts again automatically**. Do **not** “correct” this section back to PRODUCT-IDEAS. Follow this plan; note the drift in the reply. Update Guide + catalog S3-07 expected in this same P8 session.
