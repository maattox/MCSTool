...
This blanket is **TESTING**. Step **8.6.1** **shipped**: the **product** path is a **pre-built** ARM tarball copied into the user’s OCIR (**users** do not need Docker). Agents may still `fn`/`docker` on TESTING. The operator **may** produce that tar with Docker Desktop. Do not treat `fn`/`docker buildx` on a **user** PC as the installer story.
...
| **9**   | Packaging, updates, launch (old MVP Phase 8–9)             | **NEXT** — [`V1-Packaging-Plan.md`](V1-Packaging-Plan.md) **P6** (closed beta / dogfood) |


**Current NEXT step:** See [`docs/NEXT.md`](NEXT.md) — Phase **9** / Step **9.4** / [`V1-Packaging-Plan.md`](V1-Packaging-Plan.md) **P6**. Do not start **P7**.
...
**Done when:** P1–P2 **DONE**. Then operator-notes follow-on 2 (Step **8.10**); Pass 3 stays blocked until **8.10** exits and the operator says so.

**Changelog:** 2026-08-24 — Step **8.10** inserted in front of Pass 3. **8.9 remains DONE.** Living **NEXT = 8.10 P1**. Do not start 8.6.1 or 9.1. 2026-08-24 — **DONE** (P1 Core freeze + P2 assisted review UI + persist Skip + Guide). Living **NEXT** then moved to 8.10. Pass 3 **blocked**. Do not start 8.6.1 or 9.1. 2026-08-24 — **P1 DONE** (Core skip order + dependency freeze + review grouping). Living **NEXT = P2**. Pass 3 stays blocked. Do not start 8.6.1 or 9.1. 2026-08-23 — **Inserted** (docs only). Living **NEXT = P1**. Pass 3 stays blocked. Do not start 8.6.1 or 9.1.

---



## Phase 8.10 — Operator-notes follow-on 2

**Why this sits here:** Operator 2026-08-24 notes (Manager/Setup density, toasts, tab order, pack UX, VM1 color icon, MOTD styling) before QA Pass 3. Pause Pass 3 again so those paths are not tested twice.

### Step 8.10 — Operator-notes follow-on 2

**Status:** DONE  
**Depends on:** Step **8.9** DONE

**Read first**

- [`V1-Operator-Notes-Follow-On-2-Plan.md`](V1-Operator-Notes-Follow-On-2-Plan.md) protocol + **only the NEXT P-section**
- Do **not** load Pass 3, the full blueprint, or this whole V1 file

**Do**

- Implement **only** the follow-on section marked NEXT (P1 → P9). Stop after each P-section.
- UI-heavy sections must read the skills named in that plan.
- P2 and P9 are **plan-first**. P8 (VM1 icon) is PARALLEL-OK with P1–P7.
- Same TESTING permissions as this file. No `tofu apply`/`destroy`.
- Do **not** start Pass 3, **8.6.1**, or **9.1**.
...
**Done when:** P1–P9 **DONE** in [`V1-Operator-Notes-Follow-On-2-Plan.md`](V1-Operator-Notes-Follow-On-2-Plan.md). Then operator-notes follow-on 3 (Step **8.11**); Pass 3 stays blocked until **8.11** exits and the operator says so.

**Changelog:** 2026-08-24 — Step **8.11** inserted in front of Pass 3. **8.10 remains DONE.** 2026-08-24 — **DONE** (P1–P9). **P9** MOTD editor (`§` toolbar + paste, list preview, omit-name, collapsed raw; VM1 preserves codes). Pass 3 stays blocked until the operator says so. Do not start 8.6.1 or 9.1. 2026-08-24 — **P8 DONE** (VM1 color icon: `PrivateTmp` for ImageIO favicon encode; stage PNG then `install` as `mcmgr`). Living **NEXT = P9** (plan-first). Pass 3 stays blocked. Do not start 8.6.1 or 9.1. 2026-08-24 — **P7 DONE** (Setup Minecraft two-column layout: Vanilla/Modded primary; flavor/pack drop beside it). Living **NEXT = P8**. Pass 3 stays blocked. Do not start 8.6.1 or 9.1. 2026-08-24 — **P6 DONE** (Always Free explainer; OCI `<pre>` grows; OCI + budget email combined; schema v4 / 8 steps). Living **NEXT = P7**. Pass 3 stays blocked. Do not start 8.6.1 or 9.1. 2026-08-24 — **P5 DONE** (pack identity Minecraft/loader/Java dropdowns; detected extra option; catalog-fail text fallback). Living **NEXT = P6**. Pass 3 stays blocked. Do not start 8.6.1 or 9.1. 2026-08-24 — **P4 DONE** (Server inner tabs Identity / World / Modding / Change pack; mods collapsed; Usage Hours/Budget; Advanced Status/Stack/Danger; Troubleshooting two-column). Living **NEXT = P5**. Pass 3 stays blocked. Do not start 8.6.1 or 9.1. 2026-08-24 — **P3 DONE** (tab **Server**; order Whitelist → Server → Console → Usage; keep-alive + deferred catalog; confirm-then-build derived zip). Living **NEXT = P4**. Pass 3 stays blocked. Do not start 8.6.1 or 9.1. 2026-08-24 — **P2 DONE** (custom WindowChrome caption, 1040 CSS px, wordmark gone). Living **NEXT = P3**. Pass 3 stays blocked. Do not start 8.6.1 or 9.1. 2026-08-24 — **P1 DONE** (toasts bottom-left, 4s fade, start-success AutoHide). Living **NEXT = P2** (plan-first). Pass 3 stays blocked. Do not start 8.6.1 or 9.1. 2026-08-24 — **Inserted** (docs only). Living **NEXT = P1** (toasts). Pass 3 stays blocked. Do not start 8.6.1 or 9.1.

---



## Phase 8.11 — Operator-notes follow-on 3

**Why this sits here:** Operator 2026-08-24 notes (caption contrast, pin-row empty space, MOTD WYSIWYG) before QA Pass 3. Pause Pass 3 again so those paths are not tested twice.
...
- [`V1-Operator-Notes-Follow-On-3-Plan.md`](V1-Operator-Notes-Follow-On-3-Plan.md) protocol + **only the NEXT P-section**
- Do **not** load Pass 3, the full blueprint, or this whole V1 file
...
- UI-heavy sections must read the skills named in that plan.
- P4 is **plan-first**. P3 is PARALLEL-OK with P1–P2.
- Same TESTING permissions as this file. No `tofu apply`/`destroy`.
- Do **not** start Pass 3, **8.6.1**, or **9.1**.
...
**Done when:** P1–P5 **DONE** in [`V1-Manage-Sidebar-Redesign-Plan.md`](V1-Manage-Sidebar-Redesign-Plan.md). Follow-on polish is Step **8.13**.

**Changelog:** 2026-08-25 — Step **8.13** inserted (sidebar polish). This step stays **DONE**. 2026-08-25 — **DONE** (P1–P5). **P5** resize polish + Guide (Console/dock/toasts in the two-column pane; no extra inner subtabs). Living **NEXT = Step 8.13** P1. Do not start 8.6.1 or 9.1. 2026-08-25 — **P4 DONE** (Overview home tab: read-only snapshot + tab jumps). Living **NEXT = P5**. Pass 3 stays blocked. Do not start 8.6.1 or 9.1. 2026-08-25 — **P3 DONE** (About tab; caption overflow gone). Living **NEXT = P4** (plan-first). Pass 3 stays blocked. Do not start 8.6.1 or 9.1. 2026-08-25 — **P2 DONE** (combined Start/Stop + four sidebar pins). Living **NEXT = P3**. Pass 3 stays blocked. Do not start 8.6.1 or 9.1. 2026-08-25 — **P1 DONE** (two-column Manage shell, 1280 default / 920 min). Living **NEXT = P2**. Pass 3 stays blocked. Do not start 8.6.1 or 9.1. 2026-08-25 — **Inserted** (docs only). Living **NEXT = P1** (two-column Manage shell). Pass 3 stays blocked. Do not start 8.6.1 or 9.1.

---



## Phase 8.13 — Manage sidebar polish

**Why this sits here:** Operator 2026-08-25 — after the 8.12 rearrange, Manage still sits on one background. Split chrome / tabs / content into distinct panels, narrow the sidebar, equalize and compact pins, enlarge the tab list. Pause Pass 3 so the polished chrome is not tested twice. Lives on branch `UI-redesign`.

### Step 8.13 — Manage sidebar polish

**Status:** DONE  
**Depends on:** Step **8.12** DONE

**Read first**

- [`V1-Manage-Sidebar-Polish-Plan.md`](V1-Manage-Sidebar-Polish-Plan.md) protocol + **only the NEXT P-section**
- Do **not** load Pass 3, the full blueprint, or this whole V1 file
...
- Implement **only** the follow-on section marked NEXT (P1 → P2). Stop after each P-section.
- UI-heavy sections must read the skills named in that plan.
- All sections are SEQUENTIAL (shared `MainLayout` / `app.css`). Both are **agent**.
- Same TESTING permissions as this file. No `tofu apply`/`destroy`.
- Do **not** start Pass 3, **8.6.1**, or **9.1**.

**Test**

- Per the current P-section in the polish plan.

**Done when:** P1–P2 **DONE** in [`V1-Manage-Sidebar-Polish-Plan.md`](V1-Manage-Sidebar-Polish-Plan.md). Follow-on is Step **8.14**.

**Changelog:** 2026-08-25 — Step **8.14** inserted (third UI pass). This step stays **DONE**. 2026-08-25 — **DONE** (P1–P2). **P2** equal compact pins + larger tabs + Guide. Living **NEXT = Step 8.5.2** Pass 3 (**blocked**). Do not start 8.6.1 or 9.1. 2026-08-25 — **P1 DONE** (three-zone chrome + 244px flush sidebar). Living **NEXT = P2**. Pass 3 stays blocked. Do not start 8.6.1 or 9.1. 2026-08-25 — **Inserted** (docs only). Living **NEXT = P1** (three-zone chrome + narrower sidebar). Pass 3 stays blocked. Do not start 8.6.1 or 9.1.

---



## Phase 8.14 — Manage UI pass 3

**Why this sits here:** Operator 2026-08-25 — third UI redesign pass after 8.13. Window-edge 6px frame, sidebar density, pin text cutoff, Ctrl+scroll zoom, spare Overview. Pause Pass 3 so the chrome is not tested twice. Lives on branch `UI-redesign`.
...
- [`V1-Manage-UI-Pass-3-Plan.md`](V1-Manage-UI-Pass-3-Plan.md) protocol + **only the NEXT P-section**
- Do **not** load Pass 3, the full blueprint, or this whole V1 file

**Do**

- Implement **only** the follow-on section marked NEXT (P1 → P4). Stop after each P-section.
- UI-heavy sections must read the skills named in that plan. **P1** and **P4** are **plan-first**.
- P1 is PARALLEL-OK with P2 (WPF vs CSS). P3 waits on P2. P4 waits on P2.
- Same TESTING permissions as this file. No `tofu apply`/`destroy`.
- Do **not** start Pass 3, **8.6.1**, or **9.1**.
...
| [`V1-Function-Image-Plan.md`](V1-Function-Image-Plan.md) | Step **8.6.1**. **COMPLETE** (P1–P2). |
...
**Interim (until 9.1 bundles the tar):** from-source Setup may still skip the Function if the tar and Docker are both missing. TESTING agents may still `fn build`/`push` under the [Product Functions blanket](#product-functions-on-testing-blanket).

### Step 8.6.1 — Pre-built ARM image + Setup copy into OCIR

**Status:** DONE (living: [`V1-Function-Image-Plan.md`](V1-Function-Image-Plan.md) P1–P2 **COMPLETE**)  
...
**Changelog:** 2026-08-27 — **P2 DONE.** TESTING OCIR login proven (`{ns}/{identity-domain}/{IAM user name}`; user OCID **401**). Product copy without Docker; digest matched; synthetic ACTUAL SoftStop VM1 only; door up. Plan **COMPLETE**. Phase **8.6** **DONE**. Do not start 9.1 until asked. 2026-08-27 — **P1 DONE** (OCIR username derived; digest converge on Deploy/repair; Guide + developer recipe). Living **NEXT = P2**. Do not start 9.1. 2026-08-27 — Plan rewritten: developer Docker Desktop OK; users must not need Docker; GitHub Actions dropped. Living **NEXT = P1** (username + digest + Guide). 2026-08-27 — Living plan created. P13 copy path is in.
...
**Status:** DONE (living: [`V1-Packaging-Plan.md`](V1-Packaging-Plan.md) **P1–P3**)  
...
**Changelog:** 2026-08-27 — **P3 DONE** (Inno Setup 6 per-user installer + Function tar required + Guide/Release recipe). **9.1 complete.** Living **NEXT = P4** (Step **9.2** GitHub Releases update check). 2026-08-27 — **P2 DONE** (pinned OpenTofu 1.12.6 download + SHA-256). Living **NEXT = P3** (Inno installer). 2026-08-27 — **P1 DONE** (publish layout). Living **NEXT = P2** (pinned OpenTofu download). Unblocked 2026-08-27; living plan created.
...
**Status:** NEXT (living: [`V1-Packaging-Plan.md`](V1-Packaging-Plan.md) **P6**; greenfield follow-on [`V1-Greenfield-Dogfood-Follow-On-Plan.md`](V1-Greenfield-Dogfood-Follow-On-Plan.md) **COMPLETE**)  
...
| Date       | Note                                                                                                                                                                                                                                                                                                                              |
| ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-08-28 | **Follow-on P4 DONE** (SETUP-ISSUE-15: Function-stage tofu apply no longer steals the reserved play IP). Living **NEXT = P6** / Step **9.4** ([`V1-Packaging-Plan.md`](V1-Packaging-Plan.md)). Do not start P7. |
...
| 2026-08-25 | **Step 8.13 P1 DONE** (three-zone chrome + 244px flush sidebar). Living **NEXT = P2**. Pass 3 **blocked**. Do not start 8.6.1 or 9.1. |
| 2026-08-25 | **Step 8.13 inserted** (Manage sidebar polish). Living **NEXT = P1** ([`V1-Manage-Sidebar-Polish-Plan.md`](V1-Manage-Sidebar-Polish-Plan.md)). Pass 3 **blocked**. Do not start 8.6.1 or 9.1. |
| 2026-08-25 | **Step 8.12 DONE** (P1–P5). **P5** resize polish + Guide. Living **NEXT = Step 8.13** P1. Pass 3 **blocked**. Do not start 8.6.1 or 9.1. |
...
| 2026-08-24 | **Step 8.10 DONE** (P1–P9). **P9** MOTD editor (`§` toolbar + paste, list preview, omit-name, collapsed raw; VM1 preserves codes). Plan **COMPLETE**. Pass 3 **blocked** until the operator says so. Do not start 8.6.1 or 9.1. |
| 2026-08-24 | **Step 8.10 P8 DONE** (VM1 color icon: `PrivateTmp` for ImageIO favicon encode; stage PNG then `install` as `mcmgr`). Living **NEXT = P9** (plan-first). Pass 3 **blocked**. Do not start 8.6.1 or 9.1. |
| 2026-08-24 | **Step 8.10 P7 DONE** (Setup Minecraft two-column layout: Vanilla/Modded primary; flavor/pack drop beside it). Living **NEXT = P8**. Pass 3 **blocked**. Do not start 8.6.1 or 9.1. |
| 2026-08-24 | **Step 8.10 P6 DONE** (Always Free explainer; OCI `<pre>` grows; OCI + budget email combined; schema v4 / 8 steps). Living **NEXT = P7**. Pass 3 **blocked**. Do not start 8.6.1 or 9.1. |
| 2026-08-24 | **Step 8.10 P5 DONE** (pack identity Minecraft/loader/Java dropdowns; detected extra option; catalog-fail text fallback). Living **NEXT = P6**. Pass 3 **blocked**. Do not start 8.6.1 or 9.1. |
| 2026-08-24 | **Step 8.10 P4 DONE** (Server inner tabs Identity / World / Modding / Change pack; mods collapsed; Usage Hours/Budget; Advanced Status/Stack/Danger; Troubleshooting two-column). Living **NEXT = P5**. Pass 3 **blocked**. Do not start 8.6.1 or 9.1. |
| 2026-08-24 | **Step 8.10 P3 DONE** (tab **Server**; order Whitelist → Server → Console → Usage → Advanced → Troubleshooting; keep-alive + deferred catalog; confirm-then-build derived zip). Living **NEXT = P4**. Pass 3 **blocked**. Do not start 8.6.1 or 9.1. |
| 2026-08-24 | **Step 8.10 P2 DONE** (custom WindowChrome caption, 1040 CSS px, wordmark gone). Living **NEXT = P3**. Pass 3 **blocked**. Do not start 8.6.1 or 9.1. |
| 2026-08-24 | **Step 8.10 P1 DONE** (toasts bottom-left, 4s fade, start-success AutoHide). Living **NEXT = P2** (plan-first). Pass 3 **blocked**. Do not start 8.6.1 or 9.1. |
| 2026-08-24 | **Step 8.10 inserted** (operator-notes follow-on 2: density, toasts, MOTD, VM1 icon). Living **NEXT = P1** ([`V1-Operator-Notes-Follow-On-2-Plan.md`](V1-Operator-Notes-Follow-On-2-Plan.md)). Pass 3 **blocked**. Do not start 8.6.1 or 9.1. |
| 2026-08-24 | **Step 8.9 DONE** (P1 Core freeze + P2 assisted review UI + persist Skip + Guide). Living **NEXT** then moved to Step **8.10**. Pass 3 **blocked**. Do not start 8.6.1 or 9.1. |
| 2026-08-24 | **Step 8.9 P1 DONE** (Core skip order + dependency freeze + review grouping). Living **NEXT = P2**. Pass 3 **blocked**. Do not start 8.6.1 or 9.1. |
