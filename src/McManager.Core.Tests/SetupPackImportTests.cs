...
public sealed class SetupPackImportTests
{
    ...
    [Fact]
    public void Unstructured_zip_uses_a_single_client_only_confirm()
    {
        Assert.Equal(
            "Confirm all client-only mods are correctly marked",
            SetupPackImport.ClientOnlyMarksConfirmLabel);
        Assert.DoesNotContain("friend", SetupPackImport.ClientOnlyMarksConfirmLabel, StringComparison.OrdinalIgnoreCase);
    }
