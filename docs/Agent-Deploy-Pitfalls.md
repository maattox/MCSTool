...
| Same WAIT, but `cloud-init status` is **degraded done** in ~15 s, `/etc/mcmgr` missing, no `mcmgr` user, firewalld not installed | Invalid `#cloud-config`: OpenTofu `indent()` left `[Unit]` unindented in `write_files` (SETUP-ISSUE-10) | `encoding: b64` + `base64encode` for systemd units. Guest repair installs firewalld/unzip and writes the marker. Do not recreate the A1. |
...
| After successful Deploy, reserved play IP still on the **door** while Minecraft is up | Second `tofu apply` (Function image copy) re-attached `oci_core_public_ip.play` to the door secondary (SETUP-ISSUE-15) | HCL `ignore_changes = [private_ip_id]`; Setup re-runs `promote_playable.sh` after a Function copy. Do not treat Troubleshooting Park play IP as the first-run path |
