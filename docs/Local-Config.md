...
Imported Modrinth packs the Manager actually installed are copied to **`data/imported-packs/<pack>_<version>/original.mrpack`** (plus `archive.json`). That copy is for later Server Management re-download (Phase 5) — it is the original archive, not a zip of VM1 `mods/`. The product cannot reconstruct a client pack from server `mods/` (Setup strips client-only files). Gitignored with the rest of `data/`.
