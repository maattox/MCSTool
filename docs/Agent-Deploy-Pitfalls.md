...
4. **Strip CRLF on every shell script you upload from Windows** (`sed -i 's/\r$//' …`) before `bash` runs it. Shebang/`set -o pipefail` break with `\r`. Same for on-box `*.py` helpers (`paper_fill_v3.py`) — Setup's onbox `find` must include `-name '*.py'`.
