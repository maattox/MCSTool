...
public sealed class OversizedWorldBackupStoreTests
{
    ...
    {
        var store = new OversizedWorldBackupStore(new MemoryObjectStorage(), Prefixes);
        Assert.Equal("meta/oversized-world-backup.json", store.ObjectName);
    }

    [Fact]
    public async Task Get_missing_object_is_not_blocked()
    {
        var store = new OversizedWorldBackupStore(new MemoryObjectStorage(), Prefixes);
        ...
        Assert.False(result.Value.Blocked);
        Assert.Null(result.Value.Document);
    }

    [Fact]
    public async Task Put_then_get_is_blocked_with_document()
    {
        var store = new OversizedWorldBackupStore(new MemoryObjectStorage(), Prefixes);
        var doc = OversizedWorldBackupDocument.CreateBlocked(
        ...
        Assert.True(got.Value!.Blocked);
        Assert.Equal(1, got.Value.Document!.Version);
        Assert.Equal("blocked", got.Value.Document.Status);
        Assert.Equal("2026-08-11T00:00:00Z", got.Value.Document.DetectedAt);
        Assert.Equal(12_000_000_000, got.Value.Document.ArchiveSizeBytes);
        Assert.Equal(10_200_547_328, got.Value.Document.SoftCapBytes);
        Assert.Equal("archive_exceeds_soft_cap", got.Value.Document.Reason);
        Assert.Null(got.Value.ParseWarning);
    }

    [Fact]
    public async Task Malformed_json_is_still_blocked()
    {
        var memory = new MemoryObjectStorage();
        memory.Objects["meta/oversized-world-backup.json"] = "not-json"u8.ToArray();
        var store = new OversizedWorldBackupStore(memory, Prefixes);
    ...
    [Fact]
    public async Task Newer_version_is_still_blocked()
    {
        var memory = new MemoryObjectStorage();
        memory.Objects["meta/oversized-world-backup.json"] =
            """{"version": 99, "status": "blocked", "detected_at": "2026-08-11T00:00:00Z"}"""u8.ToArray();
        var store = new OversizedWorldBackupStore(memory, Prefixes);
    ...
    [Fact]
    public async Task Transport_error_is_not_treated_as_blocked()
    {
        var store = new OversizedWorldBackupStore(new FailingObjectStorage(), Prefixes);
        var got = await store.GetAsync();
    ...
    public async Task Clear_deletes_the_object_and_missing_clear_succeeds()
    {
        var memory = new MemoryObjectStorage();
        var store = new OversizedWorldBackupStore(memory, Prefixes);
        Assert.True((await store.PutAsync(OversizedWorldBackupDocument.CreateBlocked())).Succeeded);
        ...
        Assert.False(unlocked.Value!.Blocked);
