...
public partial class TroubleshootingViewModel : ViewModelBase
{
    ...
    public TroubleshootingViewModel(TroubleshootingService? service, Action<bool> setBusy)
    {
        _service = service;
        _setBusy = setBusy;
    }
    ...
    [RelayCommand]
    private async Task ParkPlayIpAsync()
    {
    ...
    [RelayCommand]
    private Task DiagnoseWaitForgeAsync() =>
        RunAsync("Diagnose wait_forge", s => s.DiagnoseWaitForgeAsync());

    [RelayCommand]
    private async Task ResetDoorStateAsync()
    {
        var confirmed = await ConfirmAsync(
            "Reset door state?",
    ...
    [RelayCommand]
    private async Task UnstickDoorAsync()
    {
        var confirmed = await ConfirmAsync(
    ...
    [RelayCommand]
    private async Task RefreshOsBudgetAsync()
    {
        var confirmed = await ConfirmAsync(
    ...
    [RelayCommand]
    private async Task HealLedgerAsync()
    {
        var confirmed = await ConfirmAsync(
            "Heal open ledger?",
    ...
    [RelayCommand]
    private Task ShowIdleStatusAsync() =>
        RunAsync("Idle timer status", s => s.ShowIdleStatusAsync());

    [RelayCommand]
    private async Task ForceEnableIdleAsync()
    {
        var confirmed = await ConfirmAsync(
    ...
    [RelayCommand]
    private async Task ReapplyNetplanAsync()
    {
        var confirmed = await ConfirmAsync(
    ...
    [RelayCommand]
    private Task DiagnoseMinecraftAsync() =>
        RunAsync("Minecraft CHDIR diagnosis", s => s.DiagnoseMinecraftChdirAsync());

    [RelayCommand]
    private async Task CopyResultLogAsync()
    {
        ...
        await clipboard.SetTextAsync(string.IsNullOrWhiteSpace(ResultLog) ? "(empty)" : ResultLog);
    ...
    private static async Task<bool> ConfirmAsync(string title, string message, string confirmButtonText)
    {
        var window = (Avalonia.Application.Current?.ApplicationLifetime as IClassicDesktopStyleApplicationLifetime)
            ?.MainWindow;
        return await ConfirmDialog.ShowAsync(window, title, message, confirmButtonText);
    }
