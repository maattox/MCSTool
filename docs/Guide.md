...
Manage reads as three panels: a dark left band (status, power, and four equal compact pins with wrapping labels), a lighter tab list that uses the leftover sidebar height with larger buttons, and the work pane on the right. Ctrl+scroll does not zoom the UI. The sidebar is narrow and flush to the left edge; the work pane takes the rest of the window.
