...
Deploy creates the compartment, network, reserved play IP, game VM, doorbell VM, shared storage, IAM, and the $1 budget Function, then installs the chosen Default Vanilla, Paper, or Modded (loader + server-side mods) server on the game VM. It can take a while. Leave the app open until the log shows success.

When Deploy **succeeds**, Setup shows **Deployment Complete** and the **reserved play IP** friends should use, with a **Copy** button. That address is already on the game VM while Minecraft is running — friends can join it without **Troubleshooting → park reserved play IP**. Click **Close** (footer) to continue to the Manager app. The deploy log stays on that page, collapsed under **Deploy log**. Reopening from **Advanced → Deploy / repair** shows **Review and deploy** again so you can re-run guest repair and copy a **newer** spend-brake image when the bundled tarball’s digest changed.
...
**Identity** on the **Server** tab: set the name and description friends see in their Minecraft server list. Each box is one list line of 59 characters (the two lines Minecraft can show). Highlight text in either field and use the toolbar to apply a color or effect (you do not type `§` codes in those boxes). Click the same color or effect again to remove it — the app rebuilds the MOTD codes from each letter’s style, so turning bold off in the middle of a colored word keeps the color on the letters around it. Counters under the Minecraft-font preview show how much of each line is used. Paste a generator `motd=` string (or type codes) in **Raw motd= string** — the preview updates as you edit. Hex/gradient colors need Paper or Spigot 1.16+; Vanilla, Forge, and Fabric ignore them. **Copy** takes the value written to the game VM. Pick a **PNG** icon (fitted to 64×64; default if you skip), and optionally edit the automated chat lines used before an idle or budget stop. Save writes the shared copy and updates the doorbell list icons (offline / starting / unavailable). **Restart** Minecraft (or **Start**) applies the in-game name, MOTD, and **color** icon — the game VM installs that PNG in the Minecraft server folder as `mcmgr` (the doorbell keeps greyscale variants for when the game VM is down).
...
The top-right **bell** opens a notification list (empty until something posts; each item can be dismissed). The **gear** opens program settings for this PC: where stack config and OpenTofu files live, plus a **Check for updates** toggle (default on). When it is on, Manager asks GitHub Releases once as it opens; if a newer version is published you get the notes and a download link — Manager never installs the update itself. Turn the checkbox off if you do not want that request. **About** (sidebar tab) has the app name, version, and a GitHub link. Bell and gear share the caption strip with min / max / close — there is no extra ☰ menu. The strip is a step darker than the rest of the window so it reads as chrome, not as more page background, and it spans the full window if you resize. Drag the empty left side of that bar to move the window. The window opens wide (~1280 CSS pixels) and can shrink to about 920. The sidebar stays a fixed width; the right pane grows and shrinks. **Console** keeps the command row at the bottom of the pane and lets the log fill leftover height. **Change pack** Install / Cancel overlay that pane only — they do not follow Overview or Server Identity / World / Modding.
